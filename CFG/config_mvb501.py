...
RADAR_CFG_LIST = [
    {'name'          : 'IWR1843_Ori',
     ...
     'ylim'          : (0.2, 2),
     'zlim'          : None,
     'pos_offset'    : (0.8, 0, 1.15),  # default pos_offset is (0, 0, 0)
     'facing_angle'  : {'angle': (0, 0, 45), 'sequence': None},  # right-hand global coord-sys, (x, y, z): [-180, 180] positive counted anti-clockwise when facing from axis end towards origin, default rotation sequence: zyx
...
VISUALIZER_CFG = {
    ...
    'auto_inactive_skip_frame': int(0.5 * RADAR_FPS),  # frames, short skip radar frames and process one when no object is detected
}

# single class instantiated, single config used
FRAME_POST_PROCESSOR_CFG = {  # post process config
    # cloud point filter para
    'FPP_global_xlim' : (0, 1),  # the x-direction limit for merged cloud points from all radars, set as [a, b), from global view
    'FPP_global_ylim' : (0, 1.5),
...
HUMAN_OBJECT_CFG = {  # human object config for each object bin
    ...
    'obj_delete_timeout'        : 30,  # second, if timeout, delete this object bin
